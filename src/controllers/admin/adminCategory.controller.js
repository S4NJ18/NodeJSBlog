...
export const allCategoryList = async (req, res)=>{
  try{
    ...
    const getCategoryList = await blogCategory.find().populate("author", "username");
    res.status(200).render('admin-category-list',{category:getCategoryList, page:"category-list", user:getUserDetails})
  }catch(err){
...
try{
   ...
   return res.status(200).render('add-category', {page:"Add Category", user: getUserDetails})
  }catch(err){
     return console.log(err)
  }
}
...
export const categoryEditGetByID = async(req, res)=>{
   ...
   try{
    ...
    return res.status(200).render("add-category",{page:"Edit Category", user:getUserDetails, category:getCategoryData})
   }catch(err){
    return err
   }
